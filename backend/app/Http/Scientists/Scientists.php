...
class Scientists extends Controller
{
    ...
    {
        $scientists = DB::select(
            "SELECT * FROM scientist WHERE phrases LIKE '%$text%'"
        );
        $array = [];
        for ($i = 0; $i < count($scientists); $i++) {
            $object = $scientists[$i];
    ...
    public function createScientist(Request $request): JsonResponse
    {
        ...
        DB::table("scientist")->insert([
            ...
            "phrases" => json_encode($body->{'phrases'}),
        ]);
        return response()->json(["inserted" => 1]);
    }

    public function updateScientist(Request $request): JsonResponse
    {
        $bodyEncode = $request->getContent();
        $body = json_decode($bodyEncode);
        DB::table("scientist")
    ...
    public function deleteScientist(string $id): JsonResponse
    {
        DB::table("scientist")->where("id", $id)->delete();
